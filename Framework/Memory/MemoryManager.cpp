...
namespace GodEngine {
	...
	static const uint32_t kMaxBlockSizes = kBlockSizes[kNumBlockSizes - 1];
	...
	int MemoryManager::Initialize() noexcept {
		...
		if (!s_bInitialized) {
			m_pBlockSizeLookup = new size_t[kMaxBlockSizes + 1];
			size_t j = 0;
			for (size_t i = 0; i <= kMaxBlockSizes; i++) {
